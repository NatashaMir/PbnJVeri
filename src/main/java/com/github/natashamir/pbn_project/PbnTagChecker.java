...
class PbnParser {
    ...
    public PbnError ParseNumber(
        ...
        for (mLength = 0; mLength < StringLength; mLength++) {
            ...
            } else if (PbnChar.IsDigit(cDigit)) {
...
public class PbnTagChecker {
    ...
    private final PbnGameTags mGameTags;
    private final PbnInherit mInheritTags;
    private final PbnImportAdmin mImportAdmin;
    private final PbnCommentAdmin mCommentAdmin;
    private final PbnParser mParser;
    private final PbnError mCheckError;
    ...
    private final String[] laScoringSystems =
            {
                    "MP",
                    "MatchPoints",
                    "IMP",
    ...
    private final String[] laScoringModifiers =
            {
                    "Butler",
                    "Butler-2",
                    "Experts",
    ...
    private boolean CheckGeneral1(
      ...
      return CheckGenEmpty(oString);              // IsOK
    }
    ...
    public PbnError CheckContract(
         ...
         */
        ...
        } else if (oString.equalsIgnoreCase("Pass")) {
            // All pass.
            lContract.GetTrump().Set(PbnTrump.NONE);
        } else if (!(('1' <= cNumber) && (cNumber <= '7'))) {
            lError.Set(PbnError.BAD_TAG_VALUE);
        } else {
            ...
            if (lError.IsOK()) {
                ...
                if (oString.equalsIgnoreCase(PbnRisk.S_NONE)) {
                    risk = PbnRisk.NONE;
                } else if (oString.equalsIgnoreCase(PbnRisk.S_DOUBLE)) {
                    risk = PbnRisk.DOUBLE;
                } else if (oString.equalsIgnoreCase(PbnRisk.S_REDOUBLE)) {
            ...
            for (iChar = 0; iChar < nChar; iChar++) {
                ...
                switch (cRank) {
                    ...
                    default:
                        ...
                        if (!lError.IsOK()) {
                            bReady = true;
                        } else if (!lRank.IsNormal()) {
                            // Not a real rank.
                            lError.Set(PbnError.BAD_RANK);
                            bReady = true;
                        } else if (!lHands.AddRank(lSuit, lRank)) {
                            // Rank already exists.
                            lError.Set(PbnError.SEVERE_TAG_VALUE);
                            bReady = true;
                        } else {
    ...
    public PbnError CheckScoreRubberHistory(
         ...
         */
            if (CheckPosNeg(laScoresNS[4], laScoresEW[4])
                ...
                lError.Set(PbnError.BAD_TAG_VALUE);
            } else if (CheckPosPos(laScoresNS[4], laScoresEW[4])) {
            ...
            } else if (CheckPosPos(laScoresNS[3], laScoresEW[3])) {
                if (!CheckTrickPointsGame(laScoresNS[2], laScoresEW[2])) {
                    lError.Set(PbnError.BAD_TAG_VALUE);
                } else if (laScoresNS[2] > laScoresEW[2]) {
    ...
    public PbnError CheckResult(
        ...
        if (lError.IsOK()) {
            ...
            if ((NrTricksEW > PbnTrick.NUMBER) ||
                ...
                lError.Set(PbnError.BAD_NUMBER);
            } else if ((PbnGen.GetVersion() == PbnGen.VERSION_10) &&
                    (NrTricksEW + NrTricksNS != PbnTrick.NUMBER)) {
                lError.Set(PbnError.BAD_NUMBER);
            }
        }
    ...
    public PbnError CallCheck(
        ...
        } else if (oString.equals("+")) {
            mImportAdmin.SetLastCall();
            lCall.SetType(PbnCall.TYPE_NONE);
        } else if (oString.equals("-")) {
            lCall.SetType(PbnCall.TYPE_NONE);
        } else if (oString.equalsIgnoreCase("AP")) {
            bAllPass = true;
        } else if (oString.equalsIgnoreCase("Pass")) {
    ...
    public PbnError IllegalCallCheck(
        ...
        } else if (oString.equalsIgnoreCase("S")) {
    ...
    public PbnError CardCheck(
         ...
         */
            if (lSuit.IsValid()) {
                ...
                } else if (!mImportAdmin.mDeal.PlayCard(mImportAdmin.mPlaySide, lCard)) {
                    lError.Set(PbnError.BAD_CARD);
                }
            }
    ...
    public PbnError TrickCheck() {
        ...
        } else if (mImportAdmin.IllegalPlayHasRevoke(lPlaySide)) {
            lError.Set(PbnError.BAD_REVOKE);
        } else if (lLeadCard.GetSuit().Is(PbnSuit.UNKNOWN)) { /*
         ...
         */
            ...
            for (int iSide = 1; iSide < PbnSide.NUMBER; iSide++) {
                ...
                if (lLeadCard.GetSuit().IsValid()) { /*
                 * Invalid lead card causes exception because its suit == -1
                 */
                    ...
                    } else if (!mImportAdmin.UnknownSideIs(lPlaySide)) { /*
    ...
    public PbnError CardIllegalCheck(
        ...
        if (oString.equals("R")) {
            if (mImportAdmin.IllegalMoveHas(PbnImportAdmin.ILLEGAL_REVOKE)) {
                lError.Set(PbnError.DOUBLE_ILLEGAL);
            } else if (mImportAdmin.IllegalMoveHas(PbnImportAdmin.ILLEGAL_LEAD)) { /*
        ...
        } else if (oString.equals("L")) {
            if (mImportAdmin.IllegalMoveHas(PbnImportAdmin.ILLEGAL_LEAD)) {
                lError.Set(PbnError.DOUBLE_ILLEGAL);
            } else if (mImportAdmin.IllegalMoveHas(PbnImportAdmin.ILLEGAL_REVOKE)) { /*
             * A revoke can not be a (irregular) lead.
             */
                lError.Set(PbnError.BAD_ILLEGAL);
            } else if (mImportAdmin.IllegalTrickHas(PbnImportAdmin.ILLEGAL_LEAD)) {
                lError.Set(PbnError.DOUBLE_ILLEGAL);
            } else {
