...
public class PbnNag {
    ...
    public boolean IsAuctionNag() {
        ...
        if (PbnGen.GetVersion() != PbnGen.VERSION_10) {
            ...
            return (CALL_EXPLAIN_FIRST <= mNag) && (mNag <= CALL_EXPLAIN_LAST);
        }
    ...
    public boolean IsPlayNag() {
        ...
        if (PbnGen.GetVersion() != PbnGen.VERSION_10) {
            ...
            return (CARD_EXPLAIN_FIRST <= mNag) && (mNag <= CARD_EXPLAIN_LAST);
        }
    ...
    public boolean IsAuctionSuffix() {
        return (AUCTION_GOOD <= mNag) && (mNag <= AUCTION_QUESTIONABLE);
    }

    public boolean IsPlaySuffix() {
        return (PLAY_GOOD <= mNag) && (mNag <= PLAY_QUESTIONABLE);
    }
}
